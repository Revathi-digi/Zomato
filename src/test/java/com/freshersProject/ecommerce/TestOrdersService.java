...
public class TestOrdersService {
	...
	public void testGetOrderHistoryDateByAsc() {
		...
		assertEquals(2,orderServiceImpl.getOrderHistoryDateByAsc(orderRequestBean).size());
	}

	@Test
	public void testGetOrderHistoryDateByDsc() {
		when(orderRepository.findAllByuserEmail(orderRequestBean.getUserEmail())).thenReturn(Stream.of(orders,order).collect(Collectors.toList()));
		assertEquals(2,orderServiceImpl.getOrderHistoryDateByDsc(orderRequestBean).size());
	}

	@Test
	public void testGetOderHistory() {
		when(orderRepository.findAllByuserEmail(orderRequestBean.getUserEmail())).thenReturn(Stream.of(orders,order).collect(Collectors.toList()));
		assertEquals(orderRequestBean.getUserEmail(),orderServiceImpl.getOderHistory(orderRequestBean).get(1).getUserEmail());
	...
	public void TestGetOrderHistoryByPaymentType() {
		...
		assertEquals(paymentOrderRequestBean.getPaymentType(), orderServiceImpl.getOrderHistoryByPaymentType(paymentOrderRequestBean).get(0).getPaymentType());
	}	
	...
	public void  testGetOrderHistoryByOrderStatus() {
		...
		assertEquals(orderStatusRequestBean.getOrderStatus(),orderServiceImpl.getOrderHistoryByOrderStatus(orderStatusRequestBean).get(0).getOrderStatus());
	}
	...
	public void testTrack() {
		...
		assertEquals(1,orderServiceImpl.track(id).getOrderId());
		//assertNotEquals(orders,orderServiceImpl.track(id).getOrderId());
	}
	...
	public void testGetRating() {
		...
		assertEquals("Thankyou for Rating", orderServiceImpl.getRating(ratingRequestBean));
	}
}
