...
@SpringBootTest
public class TestAdminServiceNegative {
	...
	Orders orders;
	ItemsRequestBean itemsRequestBean;
	...
	public void update() {
		...
		itemsRequestBean = new ItemsRequestBean(1);
	}

	@Test
	public void testSaveRestaurantDetails() {
		RestaurantDetails restaurantDetails = new RestaurantDetails(1, " Meghana Foods", 20, 400,
				"Koramangala,5th Block", 10);
		when(restaurantDetailsRepository.save(restaurantDetails)).thenReturn(restaurantDetails);
		assertNotEquals("Meghana", adminServiceImpl.saveRestaurantDetails(restaurantDetails));
	}
	...
	public void testSaveCoupons() {
		...
		assertNotEquals("APPPLY", adminServiceImpl.saveCoupons(coupons));
	}

	@Test
	public void testSaveOrderHistory() {
		Orders orders1 = new Orders(1, "Vivek@123", "Mehfil", "Veg Biryani", (float) 250.25, "Deliverd", "Paid", "Cash",
				Calendar.getInstance().getTime(), (float) 4.5);
		when(orderReposotory.save(orders1)).thenReturn(orders1);
		assertNotEquals("Mehfil", adminServiceImpl.saveOrderHistory(orders1));
	}
	...
	public void testSaveItem() {
		...
		assertNotEquals("Meghana", adminServiceImpl.saveItem(items));
	}
	...
	public void testUpdateItems() {
		...
		assertNotEquals(item, adminServiceImpl.updateItems(items));
	}
	...
	public void testUpdateCoupons() {
		...
		assertNotEquals(coupon, adminServiceImpl.updateCoupons(coupons));
	}
	...
	public void testUpdateRestaurants() {
		...
		assertNotEquals(restaurantDetail, adminServiceImpl.updateRestaurant(restaurantDetails));
	}
	...
	public void testGetAll() {
		...
		assertNotEquals(1, adminServiceImpl.getAll());
	}

	@Test
	public void testGetAllOrderHistory() {
		orders = new Orders(1, "vivek@gmail.com", "Nagajuna", "Chicken Curry", (float) 225.75, "Deliverd", "Paid",
				"Card", Calendar.getInstance().getTime(), (float) 4.5);
		Orders orders1 = new Orders(2, "sai@gmail.com", "Udupi Grand", "Poori", 50, "Deliverd", "Paid", "Cash",
				Calendar.getInstance().getTime(), (float) 4.0);
		Orders orders2 = new Orders(3, "praveen@gmail.com", "Santosh Dhaba", "Tandoori Roti", 150, "Deliverd", "Paid",
				"Cash", Calendar.getInstance().getTime(), (float) 4.0);
		...
		assertNotEquals(2, adminServiceImpl.getAllOrderHistory().size());
	}
	...
	public void testUpdateOrderStatus() {
		...
		assertNotEquals(orders, adminServiceImpl.updateOrderStatus(trackUpdateRequestBean));
	}
